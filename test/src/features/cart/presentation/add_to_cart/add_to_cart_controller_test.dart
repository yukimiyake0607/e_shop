...
void main() {
  ...
  ProviderContainer makeProviderContainer(MockCartService cartService) {
    final container = ProviderContainer(
      overrides: [cartServiceProvider.overrideWithValue(cartService)],
    );
  ...
  group('addItem', () {
    test('item added with quantity = 2, success', () async {
      ...
      final addToCartController = container.read(
      ...
      final itemQuantityController = container.read(
      ...
      verifyInOrder([
        ...
        () => addToCartListener(initialData, any(that: isA<AsyncLoading>())),
        // then the data is set with quantity: 1
        () => addToCartListener(any(that: isA<AsyncLoading>()), initialData),
      ]);
